...
function CraftingService:CraftItem(player, slotId)
    ...
    if FeatureFlags.COOK_MINIGAME_ENABLED then
        ...
        -- S-15: Phase 필드 추가 (Q1=A 요구사항)
        return {
            success = true,
            mode = "cook_minigame",
            Phase = "MINIGAME_START",             -- S-15: Phase 분리 (향후 COOK_TIME_START 등)
            slotId = slotId,
            recipeId = recipeId,
    ...
    return {
        ...
        FallbackReason = "FEATURE_DISABLED",      -- S-15: fallback 명시 (PascalCase)
        craftTime = craftTime,
...
function CraftingService:SubmitCookTap(player, slotId, sessionId, clientJudgment)
    ...
    ----------------------------------------------------------------------------
    -- S-16: UnlockOnce 헬퍼 (실패/성공 모두 슬롯 락 해제 보장)
    -- P0 버그 수정: 실패 시 슬롯 영구 잠금 방지
    ----------------------------------------------------------------------------
    local unlocked = false
    local function UnlockOnce(reason)
        if unlocked then return end
        unlocked = true
        if OrderService.SetSlotLock and type(OrderService.SetSlotLock) == "function" then
            OrderService:SetSlotLock(userId, slotId, false, reason)
            print(string.format("[CraftingService] S-16|SLOT_UNLOCK uid=%d slot=%d reason=%s",
                userId, slotId, reason))
        end
    end

    local function FailWithUnlock(err, notice)
        UnlockOnce("cook_tap_failed:" .. tostring(err))
        return { success = false, error = err, noticeCode = notice }
    end

    -- 1. 세션 존재 확인
    if not CookSessions[userId] then
        -- 세션 없음 = 락도 없었을 가능성 높음, 방어적 unlock
        UnlockOnce("cook_tap_failed:no_user_sessions")
        return { success = false, error = "no_user_sessions", noticeCode = "cook_session_error" }
    end

    local session = CookSessions[userId][slotId]
    if not session then
        UnlockOnce("cook_tap_failed:session_not_found")
    ...
    if sessionId and session.sessionId ~= sessionId then
        return FailWithUnlock("session_mismatch", "cook_session_error")
    end

    -- 2. 세션 유효성 검사
    if session.consumed then
        return FailWithUnlock("session_consumed", "cook_already_submitted")
    end

    if now > session.expiresAt then
        -- 만료된 세션 정리
        CookSessions[userId][slotId] = nil
        return FailWithUnlock("session_expired", "cook_session_expired")
    end
    ...
    if elapsed < PGConfig.MIN_COOK_TIME then
        return FailWithUnlock("too_early", "cook_too_early")
    end

    if elapsed > PGConfig.MAX_COOK_TIME then
        return FailWithUnlock("too_late", "cook_too_late")
    end
    ...
    -- 10. 슬롯 LOCK 해제 (S-16: UnlockOnce 사용으로 idempotent)
    UnlockOnce("cook_tap_submitted")
    ...
    ----------------------------------------------------------------------------
    -- S-12: CookTimePhase 분기 (PG-1.1)
    -- A) COOK_TIME_PHASE_ENABLED = false → S-11 Core (즉시 dish 생성)
    -- B) COOK_TIME_PHASE_ENABLED = true → CookTimePhase 시작 (dish 생성 지연)
    ----------------------------------------------------------------------------
    if FeatureFlags.COOK_TIME_PHASE_ENABLED then
        ------------------------------------------------------------------------
        -- (B) PG-1.1 경로: CookTimePhase 시작
        ------------------------------------------------------------------------
        local cookTimeDuration = self:GetCraftTime(player, recipeId)

        print(string.format(
            "[CraftingService] S-12|COOK_TIME_START uid=%d slot=%d recipe=%d duration=%.2f",
            userId, slotId, recipeId, cookTimeDuration
        ))

        -- CookTimePhase 완료 타이머 (S-14에서 dish 생성)
        task.delay(cookTimeDuration, function()
            self:OnCookTimePhaseComplete(player, recipeId, slotId, dishKey, session.sessionId)
        end)

        -- CookTimePhase 시작 응답 (dish 생성 전)
        return {
            success = true,
            mode = "cook_minigame",                -- 규칙 A: 절대 timer 금지
            Phase = "COOK_TIME_START",             -- S-12: Phase 분리
            CookTimeDuration = cookTimeDuration,   -- S-13: duration 전달
            slotId = slotId,
            ...
            updates = {
                inventory = DataService:GetPlayerData(player).inventory,
                craftingState = { isActive = true, phase = "COOK_TIME" },
            },
        }
    else
        ------------------------------------------------------------------------
        -- (A) PG-1 Core 경로: S-11 즉시 dish 생성 (SEALED 유지)
        ------------------------------------------------------------------------
        local giveSuccess = DataService:UpdatePlayerData(player, function(data)
            if not data.inventory then
                data.inventory = {}
        ...
        if giveSuccess then
            print(string.format("[CraftingService] S-11|COOK_DISH_CREATED uid=%d slot=%d dishKey=%s",
                userId, slotId, dishKey))
            self:SendCraftComplete(player, recipeId, dishKey, slotId)
        else
            warn(string.format("[CraftingService] S-11|COOK_DISH_FAILED uid=%d slot=%d dishKey=%s",
                userId, slotId, dishKey))
        end
...
--------------------------------------------------------------------------------
-- 3.5 OnCookTimePhaseComplete (S-14: CookTimePhase 완료 시 dish 생성)
--------------------------------------------------------------------------------

function CraftingService:OnCookTimePhaseComplete(player, recipeId, slotId, dishKey, sessionId)
    local userId = player.UserId

    -- 유저 이탈 체크
    if not player.Parent then
        warn(string.format(
            "[CraftingService] S-14|COOK_TIME_ABORT uid=%d slot=%d reason=player_left",
            userId, slotId
        ))
        return
    end
    ...
    if giveSuccess then
        print(string.format(
            "[CraftingService] S-14|COOK_TIME_COMPLETE uid=%d slot=%d dishKey=%s",
            userId, slotId, dishKey
        ))

        -- 클라이언트에 완료 알림 (CookTimePhase 완료)
        self:SendCraftComplete(player, recipeId, dishKey, slotId)

        -- CookTimePhase 완료 응답 (RE_CraftResponse로 전송)
        local Remotes = game:GetService("ReplicatedStorage"):WaitForChild("Remotes")
        local RE_CraftResponse = Remotes:FindFirstChild("RE_CraftResponse")
        if RE_CraftResponse then
            RE_CraftResponse:FireClient(player, {
                Scope = "CraftResponse",
                Success = true,
                Mode = "cook_minigame",                -- 규칙 A: 절대 timer 금지
                Phase = "COOK_TIME_COMPLETE",          -- S-14: 완료 Phase
                SlotId = slotId,
                RecipeId = recipeId,
                DishKey = dishKey,
                Updates = {
        ...
        warn(string.format(
            "[CraftingService] S-14|COOK_TIME_FAILED uid=%d slot=%d dishKey=%s",
            userId, slotId, dishKey
        ))
    end
end
