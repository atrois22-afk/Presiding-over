...
--------------------------------------------------------------------------------
-- C-09: CookTimePhase UI 래퍼 (PG-1.1)
-- UpdateCraftingState 재사용, [CookTimePhase] 로그로 fallback과 구분
--------------------------------------------------------------------------------
...
function MainHUD:StartCookTimePhase(slotId, duration, recipeId)
    ...
    print(string.format("[CookTimePhase] START slotId=%d duration=%.2f recipeId=%s",
        slotId or 0, duration or 0, tostring(recipeId or "nil")))

    -- UpdateCraftingState 호환 형식으로 호출
    -- 이 경로는 mode="cook_minigame"에서만 진입하므로 timer fallback과 충돌 없음
    self:UpdateCraftingState({
        isActive = true,
        slotId = slotId,
        recipeId = recipeId,
        startTime = startTime,
...
function MainHUD:CompleteCookTimePhase(slotId)
    print(string.format("[CookTimePhase] COMPLETE slotId=%d", slotId or 0))
