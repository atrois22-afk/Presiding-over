...
--------------------------------------------------------------------------------
-- S-33 P1: Minimal Audit (Client Sync Integrity Guard)
-- SSOT: Docs/SSOT/S33_P1_Minimal_Audit_SSOT.md
--------------------------------------------------------------------------------

-- S-33 Config
local S33_AUDIT_MIN_INTERVAL = 1.0  -- rate-limit: 동일 키 1초 내 반복 억제
local S33_ASSERT_MODE = false  -- 기본 OFF (true로 변경 시 Studio에서 error() 발동)

-- S-33 Rate-limit state: rateKey(uid|auditKey) → timestamp
local _s33AuditLastAt = {}

-- S-33 Rate-limit check: 로그 출력 여부 반환
local function _s33ShouldLog(uid, auditKey)
    local rateKey = tostring(uid) .. "|" .. auditKey
    local now = tick()
    local lastAt = _s33AuditLastAt[rateKey]
    if lastAt and (now - lastAt) < S33_AUDIT_MIN_INTERVAL then
        return false  -- 억제
    end
    _s33AuditLastAt[rateKey] = now
    return true
end

-- S-33 WARN 출력 (Production + Studio)
local function _s33Warn(uid, auditKey, details)
    if not _s33ShouldLog(uid, auditKey) then return end
    warn(string.format("[Client] S-33|AUDIT_WARN key=%s uid=%s %s", auditKey, tostring(uid), details))
end

-- S-33 ASSERT 출력 (Studio + ASSERT_MODE에서만 error, Production은 강제 WARN)
local function _s33Assert(uid, auditKey, details)
    if not _s33ShouldLog(uid, auditKey) then return end
    -- SSOT 불변식: ASSERT_MODE가 true여도 Production에서는 error 금지
    if S33_ASSERT_MODE and RunService:IsStudio() then
        error(string.format("[Client] S-33|AUDIT_ASSERT key=%s uid=%s %s", auditKey, tostring(uid), details))
    else
        warn(string.format("[Client] S-33|AUDIT_WARN key=%s uid=%s %s", auditKey, tostring(uid), details))
    end
end
...
--------------------------------------------------------------------------------
-- S-35 Full Snapshot Recovery Config (Client Self-Heal)
--------------------------------------------------------------------------------

local S35_DEBOUNCE_SECONDS = 1.0    -- audit spam 묶기
local S35_RATE_LIMIT_SECONDS = 30   -- uid 기준 전역 rate-limit
local S35_REVALIDATION_DELAY = 1.0  -- inv_negative 재검증 대기
...
local RE_RequestFullSnapshot = nil

-- S-35: Rate-limit 체크 (30초 내 재요청 차단)
local function _s35ShouldRequest()
    local now = tick()
    if (now - _s35LastRequestAt) < S35_RATE_LIMIT_SECONDS then
        return false
    end
    return true
end

-- S-35: 실제 snapshot 요청 전송
local function _s35SendRequest(auditKey, ctx)
    if not RE_RequestFullSnapshot then
        warn("[Client] S-35|SNAPSHOT_SKIP reason=remote_not_found")
        return
    end

    -- RECOVERY 상태 체크 (tutorialState)
    -- ClientData는 이 시점에서 아직 선언 전이므로, 여기서는 체크 생략
    -- (실제 호출 시점에 ClientData 접근 가능)

    if not _s35ShouldRequest() then
        warn("[Client] S-35|SNAPSHOT_SKIP reason=rate_limit")
        return
    end
    ...
    print(string.format("[Client] S-35|SNAPSHOT_REQ key=%s uid=%d rid=%d ctx=%s",
        tostring(auditKey), uid, requestId, tostring(ctx)))

    RE_RequestFullSnapshot:FireServer(auditKey, requestId)
end

-- S-35: Debounce를 거쳐 snapshot 요청 후보 등록
-- inv_negative는 재검증 로직 별도 처리
local function _s35TriggerCandidate(auditKey, ctx, isInvNegative)
    -- 이미 debounce 대기 중이면 무시 (묶기)
    if _s35PendingDebounce then
        return
    end

    if isInvNegative then
        -- inv_negative: 재검증 대기
        if _s35PendingRevalidation then
            return  -- 이미 재검증 대기 중
        end
        _s35PendingRevalidation = { dishKey = ctx, detectedAt = tick() }

        task.delay(S35_REVALIDATION_DELAY, function()
            if not _s35PendingRevalidation then return end
            local dishKey = _s35PendingRevalidation.dishKey
            _s35PendingRevalidation = nil

            -- 재검증: 여전히 음수인지 확인
            -- ClientData 접근 (이 시점에서는 정의됨)
            local data = ClientController:GetData()
            if data and data.inventory and data.inventory[dishKey] then
                if data.inventory[dishKey] < 0 then
                    -- 여전히 음수 → snapshot 요청
                    _s35SendRequest("inv_negative", dishKey)
                else
                    -- 복구됨 → 스킵
                    warn("[Client] S-35|SNAPSHOT_SKIP reason=revalidation_pass")
                end
            else
                warn("[Client] S-35|SNAPSHOT_SKIP reason=revalidation_pass")
            end
        ...
        -- 일반 auditKey: debounce 후 요청
        _s35PendingDebounce = auditKey

        task.delay(S35_DEBOUNCE_SECONDS, function()
            local pendingKey = _s35PendingDebounce
            _s35PendingDebounce = nil

            if pendingKey then
                _s35SendRequest(pendingKey, ctx)
            end
        end)
    end
end
...
local function Initialize()
    ...
    -- S-35: Full Snapshot 요청 Remote 참조 설정
    RE_RequestFullSnapshot = Remotes:FindFirstChild("RE_RequestFullSnapshot")
    if RE_RequestFullSnapshot then
        print("[Client] S-35: RE_RequestFullSnapshot 연결 완료")
    else
        warn("[Client] S-35: RE_RequestFullSnapshot not found - Self-Heal disabled")
    end
...
local function SetupSyncListener()
    ...
    RE_SyncState.OnClientEvent:Connect(function(scope, data)
        ...
        if scope == "DeliveryResponse" or scope == "CraftResponse" or
           ...
           scope == "TutorialResponse" or scope == "DiscardResponse" then
            ...
            ----------------------------------------------------------------
            -- S-30: DiscardResponse 증거 로그
            ----------------------------------------------------------------
            if scope == "DiscardResponse" then
                if normalized.success then
                    print(string.format("[Client] S-30|DISCARD_RECV success=true"))
                else
                    local errorCode = normalized.error or "unknown"
                    print(string.format("[Client] S-30|DISCARD_RECV success=false error=%s", errorCode))
                end
            end
            ...
            ----------------------------------------------------------------
            local SCOPE_ORDER = {"post_tutorial", "tutorial", "wallet", "inventory", "orders", "upgrades", "cooldown", "craftingState", "stale2Set"}
            ...
            ----------------------------------------------------------------
            -- S-33 P1 Audit (updatedScopes 기반 조건부 실행)
            ----------------------------------------------------------------
            if updatedScopes["inventory"] then
                self:AuditInventory(ClientData.inventory)
            end
            if updatedScopes["orders"] then
                self:AuditOrders(ClientData.orders)
            end
            if updatedScopes["stale2Set"] or updatedScopes["inventory"] then
                self:AuditStale2Set(ClientData.stale2Set, ClientData.inventory)
            end
        ...
        ------------------------------------------------------------------------
        -- S-35: FullSnapshotResponse (Client Self-Heal)
        -- 서버 권위 스냅샷으로 로컬 상태 전체 교체 (merge 금지)
        ------------------------------------------------------------------------
        elseif scope == "FullSnapshotResponse" then
            local snapshot = data
...
--------------------------------------------------------------------------------
-- S-33 P1: Audit Functions (updatedScopes 기반 조건부 실행)
--------------------------------------------------------------------------------

--[[
    AuditInventory: I33-P1-INV-01 검증
    - Inventory key: 재료(숫자) 또는 요리(dish_%d+) 혼합 스키마
    - key 패턴 검증: dish_ prefix가 있는 것만 ^dish_%d+$ 강제 (재료 키는 P1 범위 밖)
    - qty 검증: 모든 엔트리에 정수/음수 검사 적용
]]
function ClientController:AuditInventory(inventory)
    if inventory == nil then return end
    local uid = Players.LocalPlayer and Players.LocalPlayer.UserId or 0

    for key, qty in pairs(inventory) do
        -- qty 검증은 모든 엔트리에 적용 (재료든 요리든)
        if type(qty) ~= "number" then
            _s33Assert(uid, "inv_bad_qty", string.format("key=%s qty=%s type=%s", tostring(key), tostring(qty), type(qty)))
        else
            -- qty 정수 검사 (Lua number는 float)
            if qty % 1 ~= 0 then
                _s33Assert(uid, "inv_bad_qty", string.format("key=%s qty=%s (not integer)", tostring(key), tostring(qty)))
            end
            -- qty 음수 검사
            if qty < 0 then
                _s33Assert(uid, "inv_negative", string.format("key=%s qty=%d", tostring(key), qty))
                -- S-35: inv_negative는 재검증 후 snapshot 요청
                _s35TriggerCandidate("inv_negative", key, true)
            end
        end

        -- key 패턴 검증: dish_ prefix가 있는 것만 검사 (재료 키는 스킵)
        if type(key) == "string" and string.match(key, "^dish_") then
            if not string.match(key, "^dish_%d+$") then
                _s33Assert(uid, "inv_bad_key", string.format("dishKey=%s", tostring(key)))
            end
        end
    end
end
...
--[[
    AuditOrders: I33-P1-ORD-01 검증
    - reservedDishKey는 nil 또는 ^dish_%d+$ 패턴
    - reservedStaleLevel은 0|1|2
]]
function ClientController:AuditOrders(orders)
    if orders == nil then return end
    local uid = Players.LocalPlayer and Players.LocalPlayer.UserId or 0

    for slotId, order in pairs(orders) do
        if type(order) == "table" then
            -- reservedDishKey 패턴 검사 (nil 허용)
            local rdk = order.reservedDishKey
            if rdk ~= nil then
                if type(rdk) ~= "string" or not string.match(rdk, "^dish_%d+$") then
                    _s33Assert(uid, "ord_bad_reserved_key", string.format("slotId=%s reservedDishKey=%s", tostring(slotId), tostring(rdk)))
                    -- S-35: snapshot 요청 트리거
                    _s35TriggerCandidate("ord_bad_reserved_key", slotId, false)
                end
            end

            -- reservedStaleLevel 범위 검사 (nil 허용, 있으면 0/1/2)
            local rsl = order.reservedStaleLevel
            if rsl ~= nil then
                if type(rsl) ~= "number" or (rsl ~= 0 and rsl ~= 1 and rsl ~= 2) then
                    _s33Assert(uid, "ord_bad_shape", string.format("slotId=%s reservedStaleLevel=%s", tostring(slotId), tostring(rsl)))
                    -- S-35: snapshot 요청 트리거
                    _s35TriggerCandidate("ord_bad_shape", slotId, false)
                end
            end
        end
    end
...
--[[
    AuditStale2Set: I33-P1-ST2-01 검증
    - stale2Set의 모든 key는 inventory에 존재하고 qty >= 1
    - 트리거: updatedScopes["stale2Set"] OR updatedScopes["inventory"]
]]
function ClientController:AuditStale2Set(stale2Set, inventory)
    if stale2Set == nil then return end  -- 초기화 전 호출 방지
    if inventory == nil then return end
    local uid = Players.LocalPlayer and Players.LocalPlayer.UserId or 0

    for dishKey, _ in pairs(stale2Set) do
        local qty = inventory[dishKey]
        if qty == nil then
            _s33Warn(uid, "st2_missing_inventory", string.format("dishKey=%s (not in inventory)", tostring(dishKey)))
            -- S-35: snapshot 요청 트리거
            _s35TriggerCandidate("st2_missing_inventory", dishKey, false)
        elseif qty < 1 then
            _s33Warn(uid, "st2_zero_qty", string.format("dishKey=%s qty=%d", tostring(dishKey), qty))
            -- S-35: snapshot 요청 트리거
            _s35TriggerCandidate("st2_zero_qty", dishKey, false)
        end
    end
end
