...
function RemoteHandler:ConnectRemotes()
    ...
    ---------------------------------------------------------------------------
    -- S-35: RE_RequestFullSnapshot (RemoteEvent - Client Self-Heal)
    ---------------------------------------------------------------------------
    local RE_RequestFullSnapshot = Remotes:FindFirstChild("RE_RequestFullSnapshot")
    if RE_RequestFullSnapshot then
        -- S-35 Rate-limit: uid 기준 30초
        ...
        RE_RequestFullSnapshot.OnServerEvent:Connect(function(player, reasonKey, requestId)
            ...
            -- Rate-limit check
            local lastTime = _s35RateLimitCache[uid]
            if lastTime and (now - lastTime) < S35_RATE_LIMIT_SECONDS then
                warn(string.format("[RemoteHandler] S-35|SNAPSHOT_DROP uid=%d reason=rate_limit", uid))
                return
            end
            _s35RateLimitCache[uid] = now

            LogServerRecv(player, "RequestFullSnapshot")

            -- BuildFullSnapshot (bad_state 체크 포함)
            local snapshot = Services.DataService:BuildFullSnapshot(player, requestId, reasonKey)
            if not snapshot then
                warn(string.format("[RemoteHandler] S-35|SNAPSHOT_DROP uid=%d reason=bad_state", uid))
                return
            end
            ...
            print(string.format("[RemoteHandler] S-35|SNAPSHOT_SERVE uid=%d bytes=%d reason=%s", uid, bytes, tostring(reasonKey)))
            RE_SyncState:FireClient(player, "FullSnapshotResponse", snapshot)
        end)
        print("[RemoteHandler] RE_RequestFullSnapshot 연결 완료 (S-35)")
    else
        warn("[RemoteHandler] RE_RequestFullSnapshot RemoteEvent not found - S-35 disabled")
    end
