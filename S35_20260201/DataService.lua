...
--[[
    S-35: Full Snapshot 구성 (Client Self-Heal용)
    SSOT: Docs/SSOT/S35_Full_Snapshot_Recovery_SSOT.md

    @param player: Player 객체
    @param requestId: 클라이언트 요청 ID (echo용)
    @param reasonKey: 트리거된 auditKey
    @return snapshot 테이블 또는 nil (bad_state)

    포함 필드: inventory, orders, Stale2Keys, wallet, cooldown, serverTime, requestId
    주의: deep copy로 정규화된 테이블 반환 (참조 공유 금지)
]]
function DataService:BuildFullSnapshot(player, requestId, reasonKey)
    local uid = player.UserId
    local playerData = self:GetPlayerData(player)

    -- bad_state 체크
    if not playerData then
        warn(string.format("[DataService] S-35|SNAPSHOT_DROP uid=%d reason=no_player_data", uid))
        return nil
    end

    -- RECOVERY 상태 체크 (tutorialState가 RECOVERY면 스킵)
    if playerData.tutorialState == "RECOVERY" then
        warn(string.format("[DataService] S-35|SNAPSHOT_DROP uid=%d reason=recovery", uid))
        return nil
    end

    -- Deep copy로 정규화된 스냅샷 구성
    local snapshot = {
        requestId = requestId,
        reasonKey = reasonKey,
        serverTime = os.time(),
    ...
    -- inventory (혼합 스키마: 숫자키=재료, dish_%d+=요리)
    snapshot.inventory = {}
    ...
    -- Stale2Keys (배열)
    snapshot.Stale2Keys = self:GetStale2Keys(player)

    -- wallet
    snapshot.wallet = {
        coins = playerData.wallet and playerData.wallet.coins or 0,
    }
    ...
    -- 실제 cooldown 상태가 있으면 복사
    if playerData.cooldown then
        snapshot.cooldown.remainingSeconds = playerData.cooldown.remainingSeconds or 0
        snapshot.cooldown.isReady = playerData.cooldown.isReady ~= false
        snapshot.cooldown.tutorialActive = playerData.cooldown.tutorialActive or false
    end

    -- 로그
    local bytes = 0 -- 대략적인 크기 추정 생략 (디버깅 시 추가 가능)
    print(string.format("[DataService] S-35|SNAPSHOT_BUILD uid=%d rid=%s reasonKey=%s serverTime=%d",
        uid, tostring(requestId), tostring(reasonKey), snapshot.serverTime))
